...
void write(const double *f1, const double *f2, const double *rho, const double *ux, const double *uy, const int t){
    ...
    H5LTmake_dataset(file, "/f1" , 3, &DIM, H5T_NATIVE_DOUBLE, f1);
    H5LTmake_dataset(file, "/f2", 3, &DIM, H5T_NATIVE_DOUBLE, f2);
    H5LTmake_dataset(file, "/rho", 3, &DIM, H5T_NATIVE_DOUBLE, rho);
    H5LTset_attribute_int(file, "/rho", "NX", &NX, 1);
    H5LTset_attribute_int(file, "/rho", "NY", &NY, 1);
    H5LTmake_dataset(file, "/ux", 3, &DIM, H5T_NATIVE_DOUBLE, ux);
    H5LTmake_dataset(file, "/uy", 3, &DIM, H5T_NATIVE_DOUBLE, uy);
